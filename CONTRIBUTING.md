...
`cargo nextest` is required for `nanocode test`
...
2. Remember that `ankh-types` DTO changes force each leaf to regenerate its
   ...
   their own product declarations); `nanocode tidy` in each leaf checks this.
3. Run `nanocode check-siblings` from Ankh before committing. It runs each
   present leaf's own `nanocode tidy` and `nanocode test` gates against the
