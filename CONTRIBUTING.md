...
ncode tidy        # Rust + frontend formatting and lints
ncode test        # Rust tests (auto-starts Postgres) + frontend smoke tests
cargo xtask demo --seed # run the full stack locally with seeded demo identities
```

`cargo nextest` is required for `ncode test`
...
2. Remember that `ankh-types` DTO changes force each leaf to regenerate its
   ...
   their own product declarations); `ncode tidy` in each leaf checks this.
3. Run `cargo xtask check-siblings` from Ankh before committing. It runs each
   present leaf's own `ncode tidy` and `ncode test` gates against the
